  ...
  try {
    ...
    compiler.hooks.done.tap('DevServer', (stats) => {
      if (stats.hasErrors()) {
        ...
        logger.info('\n' + chalk.bold('Element Development Server'));
        logger.info('─'.repeat(40));
        logger.info(`Element: ${chalk.cyan(manifest.name)}`);
        logger.info(`Version: ${chalk.cyan(manifest.version)}`);
        logger.info(`Category: ${chalk.cyan(manifest.category)}`);
        logger.info('─'.repeat(40));
        logger.info(`Local: ${chalk.cyan(`http://${options.host}:${port}`)}`);
        logger.info(`Preview: ${chalk.cyan(`http://${options.host}:${port}/preview`)}`);
        logger.info(`API Docs: ${chalk.cyan(`http://${options.host}:${port}/api-docs`)}`);
        logger.info('─'.repeat(40));
        logger.info('Use ' + chalk.cyan('Ctrl+C') + ' to stop the server\n');
      }
    });
    ...
    if (options.open) {
      setTimeout(() => {
        open(`http://${options.host}:${port}/preview`);
      }, 1000);
    }

    // Watch for manifest changes
    const watcher = chokidar.watch(['manifest.json', 'src/**/*'], {
      cwd: projectDir,
      ignoreInitial: true
    });
    ...
    watcher.on('change', (file) => {
      ...
      if (file === 'manifest.json') {
        ...
        try {
          const newManifest = fs.readJsonSync(manifestPath);
          Object.assign(manifest, newManifest);
        } catch (error) {
          logger.error('Failed to reload manifest:', error.message);
        }
      }
    });
...
function generatePreviewHTML(manifest: any, port: number): string {
  return `
<!DOCTYPE html>
<html lang="en">
<head>
